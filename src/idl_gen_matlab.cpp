...
struct IDLOptionsMATLAB : public IDLOptions {
  // All fields start with 'oct_' prefix to distuinguish from the base options
  IDLOptionsMATLAB(const IDLOptions& opts) : IDLOptions(opts) {}
};

class MATLABGenerator : public BaseGenerator {
public:
  MATLABGenerator(const Parser &parser, const std::string &path,
                 const std::string &file_name, IDLOptionsMATLAB opts)
...
//code_.Clear();
    code_.clear();
    code_ += "% MATLAB code " + std::string(FlatBuffersGeneratedWarning()) + nl;

    // We will probably write it to a octave script file, start with empty statement:
    code_ += "classdef " + ClassdefName() + nl;
    code_ += "methods(Static)" + nl;
...
private:
 //CodeWriter code_;
  ...
  std::string ClassdefName() { return file_name_ + "_generated"; }
  ...
  std::string GenReadUint32AtIdx(const std::string& idx)
  {
    ...
    return "flatbuffers.ReadUint32(" + GenReadBytesAtIndex(idx, 4U) + ")";
  }
  // Generate .m for a function that reads an int32 from the specified index
  std::string GenReadInt32AtIdx(const std::string& idx)
  {
    return "flatbuffers.ReadInt32(" + GenReadBytesAtIndex(idx, 4U) + ")";
  }
  // Generate .m for a function that reads a uint16 from the specified index
  std::string GenReadUint16AtIdx(const std::string& idx)
  {
    return "flatbuffers.ReadUint16(" + GenReadBytesAtIndex(idx, 4U) + ")";
  }
  ...
  std::string GenReadUint32(const std::string& str) {return "flatbuffers.ReadUint32(" + str + ")"; }

  std::string GenWriteString(const std::string &str){ return "flatbuffers.WriteString(" + str + ")"; }
  std::string GenWriteUint16(const std::string &str){ return "flatbuffers.WriteUint16(" + str + ")"; }
  std::string GenWriteUint32(const std::string &str){ return "flatbuffers.WriteUint32(" + str + ")"; }
  std::string GenWriteInt32(const std::string &str){ return "flatbuffers.WriteInt32(" + str + ")"; }
...
// In Octave, it's dynamically typed
  ...
  std::string GenPackVector(const std::string& field_name, const std::string& struct_field, const Type& type)
  {
    ...
    if(bGenForLoop)
    {
      ...
      pack += "BufOutline = [BufOutline, " + GenWriteUint32("N_" + field_name) + "];" + nl;
    ...
    case BASE_TYPE_STRING:
    {
      ...
      pack += "offTo_" + field_name + "_k = N_" + field_name + " * 4 + 4 - 4 * k + length(VecOutline_" + field_name + ");" + nl;
      pack += "VecOffsets_" + field_name + " = [VecOffsets_" + field_name + ", " + GenWriteUint32("offTo_" + field_name + "_k") + "];" + nl;
      pack += "VecOutline_" + field_name + " = [VecOutline_" + field_name + ", " + GenWriteString("str_" + field_name + "_k") + "];" + nl;
      break;
    }
    default:
    {
      ...
      pack += "Bytes = " + GenWriteUint32("length(T." + field_name + ")") + ";" + nl;
      // Pack the vector bytes
      pack += "Bytes = [Bytes, reshape(typecast(" + octave_type + "(T." + field_name + "), \"uint8\"), 1, [])];" + nl;
  ...
  std::string GenUnpackStruct(const std::string& field_name, const std::string& struct_field, const Type& type)
  {
    std::string struct_name = NativeName(Name(*type.struct_def), type.struct_def, opts_);
    std::string unpack;
    unpack += struct_field + " = " + ClassdefName() + "." + GenUnpackFunctionName(struct_name) + "(b, " + GenIdxFieldNameOff(field_name) + ")";
    return unpack;
  }
...
// TODO
  ...
  void GenPackTable(const StructDef& struct_def)
  {
    ...
    */
    ...
    str += "BufInline = " + GenWriteInt32("0") + ";" + nl;
    ...
    str += "BufInline(idx:idx + 3) = " + GenWriteUint32("offsOutline(k) + length(BufInline)") + ";" + nl;
    str += "end" + nl;
...
bool GenerateMATLAB(const Parser &parser, const std::string &path,
  const std::string& file_name)
{
  matlab::IDLOptionsMATLAB opts(parser.opts);

  matlab::MATLABGenerator generator(parser, path, file_name, opts);
